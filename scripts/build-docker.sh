...
IMG_REPO=${PROJECT_SLUG:-rest-abs-challenge}
IMG_VERSION=${IMG_VERSION:-$(./scripts/get-version.sh)}
IMG_SUBTAG=${IMG_SUBTAG:-}
IMG_PLATFORM=${IMG_PLATFORM:-$(uname -m)}
DOCKERFILE_PATH=${DOCKERFILE_PATH:-./src/abs_challenge/challenge/Dockerfile}
CONTEXT_PATH=${CONTEXT_PATH:-./src/abs_challenge/challenge}
...
OPTIONS:
    ...
    -r, --repo [REPO]            Image repository. Default: rest-abs-challenge
    -v, --version [VERSION]      Image version tag.
    -s, --subtag [SUBTAG]        Additional image subtag.
    -d, --dockerfile [PATH]      Path to Dockerfile. Default: ./src/abs_challenge/challenge/Dockerfile
    -t, --context-path [PATH]    Build context path. Default: ./src/abs_challenge/challenge
