...
SERVER_URLS = ["http://runner-1:8080", "http://runner-2:8080"]
...
def test_total_and_per_framework_counts():
    ...
    assert len(schedule) == len(FRAMEWORKS) * 5 * len(SERVER_URLS) + 1
    by_name = Counter(unit["name"] for unit in schedule)
    for framework in FRAMEWORKS:
        assert by_name[framework] == 5 * len(SERVER_URLS)
...
def test_deterministic_order_when_not_shuffled():
    ...
    assert [
        (unit["name"], unit["headless"], unit["server_url"]) for unit in schedule
    ] == [
        ("a", False, SERVER_URLS[0]),
        ("a", True, SERVER_URLS[0]),
        ("a", False, SERVER_URLS[1]),
        ("a", True, SERVER_URLS[1]),
        ("b", False, SERVER_URLS[0]),
        ("b", True, SERVER_URLS[0]),
        ("b", False, SERVER_URLS[1]),
        ("b", True, SERVER_URLS[1]),
    ]
...
def test_order_numbers_are_assignable_contiguously():
    ...
    for unit in schedule:
        assert set(unit.keys()) == {"name", "headless", "server_url"}
...
def test_payload_manager_deterministic_when_shuffle_off(monkeypatch):
    ...
    assert [
        (task["name"], task["headless"], task["server_url"])
...
def test_payloads_from_multiple_servers_are_scored_independently(monkeypatch):
    ...
    assert manager.submitted_payloads[0]["server_url"] == SERVER_URLS[0]
    assert manager.submitted_payloads[1]["server_url"] == SERVER_URLS[1]
